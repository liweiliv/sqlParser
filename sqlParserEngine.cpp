...
public:
    ...
    SQLWordArray(const SQLWordArray&s) :
            SQLWord( SQL_ARRAY, s.m_optional), m_or(s.m_or), m_loop(
                    s.m_loop)
    {
...
private:
    ...
    map<uint32_t, SQLWord *> m_parseTreeHead;
    void * m_funcsHandel;
    SQLWord* loadSQlWordFromJson(jsonValue *json)
    {
       ...
       if (NULL != (value = obj->get("INCLUDE")))
        ...
        if ((value = obj->get("K")) != NULL)
        {
            ...
            else if (strncmp(static_cast<jsonString*>(value)->m_value.c_str(),"_S_:", 4) == 0)
                s = new SQLSingleWord(optional, SQLSingleWord::STRING,static_cast<jsonString*>(value)->m_value.c_str() + 4);
            else if (strncmp(static_cast<jsonString*>(value)->m_value.c_str(),"_C_:", 4) == 0)
                s = new SQLSingleWord(optional, SQLSingleWord::CHAR, static_cast<jsonString*>(value)->m_value.c_str() + 4);
            else
            {
...
LOAD:
    ...
    int LoadParseTree(const char *config)
    {
        ...
        while (true)
        {
            ...
            s->include();
            pair<map<uint32_t, SQLWord *>::iterator, bool> i = m_parseTree.insert(pair<uint32_t, SQLWord*>(id, s));
            if(!i.second)
            {
                printf("%s has the same id [%d] with %s\n",v->toString().c_str(),id,i.first->second->m_comment.c_str());
                delete v;
                return -1;
            }
            if(head)
...
int main(int argc,char * argv[])
{
   ...
   if(0!=p.LoadParseTreeFromFile("ParseTree"))
    ...
    char * sql = NULL;
    if(argc==1)
        sql =(char*)"create table `appromo_prize_redeem_order_070` ( `id` bigint not null auto_increment comment '主键' , `gmt_create` timestamp(6) not null default current_timestamp(6) comment '创建时间' , `gmt_modified` timestamp(6) not null default current_timestamp(6) comment '修改时间' , `prize_redeem_order_id` varchar(64) not null comment '奖品核销id' , `biz_no` varchar(64) not null comment '活动申请id' , `user_id` varchar(64) not null comment '用户id' , `activity_id` varchar(64) not null comment '活动id' , `prize_detail_id` varchar(64) not null comment '奖品明细id' , `prize_id` varchar(64) not null comment '奖品id' , `prize_type` varchar(16) not null comment '奖品类型' , `biz_date` timestamp(6) not null comment '核销时间' , `amount` bigint not null comment '数量' , `currency` varchar(8) comment '币种' , `extend_info` varchar(4096) comment '扩展信息' , primary key ( id ) , unique key `uk_prize_redeem_order_id` ( prize_redeem_order_id ) , key `idx_bizNo_userId_activityId_prizeType_prizeDetailId` ( biz_no , user_id , activity_id , prize_type , prize_detail_id ) ) comment = '奖品核销流水表';";
        else
    {

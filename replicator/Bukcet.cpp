...
struct BSSQueueNode
{
    ...
    atomic<BSSQueueNode> * next;
    atomic<uint8_t> start;
    atomic<uint8_t> end;
    static BSSQueueNode *createNewNode(mempool * pool)
    {
        BSSQueueNode * node = static_cast<BSSQueueNode*>(pool->alloc());
...
struct BSSQueue
{
    atomic< BSSQueueNode<T> * > _head;
    atomic< BSSQueueNode<T> * > _end;
    void * userData;
    mempool * pool;
    inline void init(mempool * pool,void * userData)
    {
        ...
        _head.store(BSSQueueNode<T>::createNewNode(pool),std::memory_order_relaxed);
    ...
    {
        BSSQueueNode<T> * end = _end.load(std::memory_order_relaxed);
        if(end->end == BSSQueueNodeBufSize/sizeof(T)-1)
        {
            BSSQueueNode<T> * tmp = BSSQueueNode<T>::createNewNode(pool);
            tmp->buf[0] = value;
    ...
    {
        BSSQueueNode<T> * head = _head.load(std::memory_order_relaxed);
        ...
        {
            if(start==head->end.load(memory_order_acquire))
                return NULL;
            else
                return head->buf[start];
        }
    ...
    inline const T  head_PushOnlyThread()
    {
